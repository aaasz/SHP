...
void test_function_eight(co_stream s_in, co_stream s_out1, co_stream s_out2)
{
    int i;
    int hi = 10000000;
	int32   	err;
    ...
    HW_STREAM_WRITE(test_function_eight, s_out1, hi);
    HW_STREAM_WRITE(test_function_eight, s_out2, hi);
    ...
    StartIF_TS = XTmrCtr_GetValue (&TimerInst, 0);
#endif
   	HW_STREAM_READ(test_function_eight, s_in, hi, err);
    ...
    HW_STREAM_CLOSE(test_function_eight, s_in);
    HW_STREAM_CLOSE(test_function_eight, s_out1);
    HW_STREAM_CLOSE(test_function_eight, s_out2);
}
