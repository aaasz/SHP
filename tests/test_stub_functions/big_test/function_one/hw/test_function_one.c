...
void test_function_one(co_stream s_in, co_stream s_out)
{
    int i;
    int hi = 10000000;
	int32   	err;
    ...
    HW_STREAM_WRITE(test_function_one, s_out, hi);
    ...
    StartIF_TS = XTmrCtr_GetValue (&TimerInst, 0);
#endif
   	HW_STREAM_READ(test_function_one, s_in, hi, err);
    ...
    HW_STREAM_CLOSE(test_function_one, s_in);
    HW_STREAM_CLOSE(test_function_one, s_out);
}
